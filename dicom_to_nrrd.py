...
class Img_Series:
    def __init__(self, data_dir, modality, img_type=None, load_method=None, patient_num=None, trim=True):
        if img_type is None:
...
def get_diffusion_image_metadata(data_dir, sequence_pattern=r'\*ep_b[0-9]*#?[1-6]?$'):
    #this loads returns lists of metadata in order of ipp sorted by b value in a dictionary. goes by the sequence name *ep_b[####]#[1-6]
    files = os.listdir(data_dir)
    sequence_metadata_dict = {}
    for file in files:
        file_path = os.path.join(data_dir, file)
        metadata = pydicom.dcmread(file_path)
        try:
            sequence_name = metadata[0x0018,0x0024].value
        except:
            continue
        if re.match(sequence_pattern, sequence_name):
            # for tag in metadata:
            #     print(f"tag {tag}")
            b_value = metadata[0x0019,0x100c].value
            if b_value in sequence_metadata_dict:
                if sequence_name[-2:] == "b0":
                    sequence_metadata_dict[b_value][1].append(metadata)    
                else:  
                    sequence_metadata_dict[b_value][int(sequence_name[-1])].append(metadata)    #last char of name is the diffusion direction (1-6)
            else:
                sequence_metadata_dict[b_value] = {}
                for num in range(1,7):
                    sequence_metadata_dict[b_value][num] = []
                if sequence_name[-2:] == "b0":
                    sequence_metadata_dict[b_value][1].append(metadata)    
                else:   
                    sequence_metadata_dict[b_value][int(sequence_name[-1])].append(metadata)
    #now sort by the ipp for each:
    for b in sequence_metadata_dict:
        for diff_dir in sequence_metadata_dict[b]:
            sequence_metadata_dict[b][diff_dir].sort(key=lambda val: val[0x0020, 0x0032].value[2])
    #now need to sort by slice location, then average over diffusion directions and averages (all at same slice with same b)
    # first average over averages
    b_value_image_dict = {}
    for b_value in sequence_metadata_dict:
        img_slices = {}
        for num in range(1,7):
            #sort according to ipp  [2]  
            for metadata in sequence_metadata_dict[b_value][num]: 
                ipp = str(metadata[0x0020, 0x0032].value)
                if ipp in img_slices:
                    img_slices[ipp].append((metadata.pixel_array * float(metadata.RescaleSlope)) + float(metadata.RescaleIntercept))
                else:
                    img_slices[ipp] = [metadata.pixel_array * float(metadata.RescaleSlope) + float(metadata.RescaleIntercept)]
        #now average over images:
        b_value_image_dict[b_value] = {}
        for ipp in img_slices:
            avg_img = np.mean(np.array(img_slices[ipp]), axis=0)
            avg_img[avg_img < 10] = np.nan
            img_slices[ipp] = avg_img
            b_value_image_dict[b_value][ipp] = avg_img

    #return images as well as the relevant dicom tags
    meta_attrs = {}
    meta_attrs["iop"] = metadata.ImageOrientationPatient
    meta_attrs["modality"] = metadata.Modality
    meta_attrs["acquisition_date"] = metadata.AcquisitionDate
    meta_attrs["slice_thickness"] = metadata.SpacingBetweenSlices
    meta_attrs["pixel_spacing"] = metadata.PixelSpacing
    meta_attrs["patient_name"] = metadata.PatientName
    #get origin (smallest z value's ipp)
    import ast
    ipps = list(img_slices.keys())
    for i in range(len(ipps)):
        ipps[i] = ast.literal_eval(ipps[i])
    ipps.sort(key=lambda val: val[2])
    meta_attrs["origin"] = ipps[0]
    meta_attrs["ipps"] = ipps
    return b_value_image_dict, meta_attrs

def fits_opener(file_path : str):
    #this function takes the file path to a fits file and opens the data, storing the image as a multidimensional numpy array. also make an array for the coordinates of each voxel. also return b value
    #returns 
    #img_array (2d), img_coords (5d (3 for x,y,z))
    data = fits.open(file_path)
    #this will have a list of the 32 slices for the given b value
    img_data_list = []
    #patient_coord_list = []
    for idx, image in enumerate(data):
        img_data = {}
        ygormeta = []#image.header["YGORMETA"]
        for i, item in enumerate(image.header):
            if item == "YGORMETA":
                ygormeta.append(image.header[i])
        ygorofst = image.header["YGOROFST"]
        ygorcolu = image.header["YGORROWU"] #    *** these two currently switched so variables are defined oppositely 
        ygorrowu = image.header["YGORCOLU"]
        voxel_dimensions = [float(image.header["YGORPXLX"]), float(image.header["YGORPXLY"]), float(image.header["YGORPXLZ"])]

        #now get the b value from the metadata list
        b_value = None
        for item in ygormeta:
            if "b_value" in item.lower():
                b_value = int(re.findall(r'\d+', item)[-1])
                img_data["b_value"] = b_value
        if b_value is None:
            print(f"no b-value found in the metadata for {file_path}")
            return None 
            

        # pos_offset = ygorofst.replace("(", "").replace(")", "").split(",")   
        # pos_offset = [float(val) for val in pos_offset]     
        # ipp = pos_offset


        # column_cos = ygorcolu.replace("(", "").replace(")", "").split(",")   
        # column_cos = [float(val) for val in column_cos]

        # row_cos = ygorrowu.replace("(", "").replace(")", "").split(",")   
        # row_cos = [float(val) for val in row_cos]

        # iop = row_cos + column_cos
        
        #get the ipp 
        for tag in ygormeta:
            if "ImagePositionPatient" in tag:
                ipp = [float(val.strip()) for val in tag.split("=")[1].strip().split("\\")]
                img_data["ipp"] = ipp
                break

        for tag in ygormeta:
            if "PixelSpacing" in tag:
                ipp = [float(val.strip()) for val in tag.split("=")[1].strip().split("\\")]
                img_data["pixel_spacing"] = ipp
                break
        #get the iop
        for tag in ygormeta:
            if "ImageOrientationPatient" in tag:
                iop = [float(val.strip()) for val in tag.split("=")[1].strip().split("\\")]
                img_data["iop"] = iop
                break    
        #get patient name
        for tag in ygormeta:
            if "PatientsName" in tag:
                patient_name = tag.split("=")[1].strip() 
                img_data["patient_name"] = patient_name
                break
        #get slice thickness
        for tag in ygormeta:
            if "SpacingBetweenSlices" in tag:
                slice_thickness = float(tag.split("=")[1].strip())    
                img_data["slice_thickness"] = slice_thickness    
                break   

        #get acquisition date
        for tag in ygormeta:
            if "AcquisitionDate" in tag:
                acquisition_date = tag.split("=")[1].strip()    
                img_data["acquisition_date"] = acquisition_date
                break     
        img_array = np.array(image.data)
        #img_array currently reversed in fits file so flip
        img_array = np.flip(img_array, axis=0)

        #replace signal<2 voxels outside body with nan
        img_array[img_array<2] = np.nan
        img_data["image"] = img_array
        img_data_list.append(img_data)
    #now sort image data by slice
    img_data_list.sort(key=lambda val: val["ipp"][2])
        

    return img_data_list

def fits_opener_adc(file_path : str):
    #this function takes the file path to a fits file and opens the data, storing the image as a multidimensional numpy array. also make an array for the coordinates of each voxel. also return b value
    #returns 
    #img_array (2d), img_coords (5d (3 for x,y,z))
    data = fits.open(file_path)
...
def prepare_all_diffusion_images(data_path, load_type_dict):
    #here we process all images for running ivim model. Some are wip, some non wip and some simple adc loading. load_type_dict tells what type of loading to do for each patient. 
    
    for patient in reversed(list(load_type_dict.keys())):
        print(f"Preparing images for {patient}")
        img_dict = {}
        for scan_type in load_type_dict[patient]:
            print(f"Preparing images for {scan_type} - RT")
            load_type = load_type_dict[patient][scan_type]

            img_series = Img_Series(os.path.join(data_path, patient, scan_type), "MR", img_type="diffusion", load_method=load_type, patient_num=patient[-2:])
            img_dict["diffusion"] = img_series
            #save the array
            with open(os.path.join(os.getcwd(), "processed_data", str(patient + scan_type + "diffusion_imgs")), "wb") as fp:
                pickle.dump(img_dict ,fp)
            #fits_data.writeto(os.path.join(os.getcwd(), "temp", str(patient + scan_type + "fits")))
...
def save_img_as_nrrd(img_series, save_path, suv_factors=None, deblurred=False):
    if suv_factors==None:
        img_file_name =  "img.nrrd"

        if type(save_path) == str:
            img_save_path = os.path.join(save_path, img_file_name)
...
def save_img_and_mask_as_nrrd(img_series, structure_name, structure, save_paths, suv_factors=None, segmentation_types=["reg"], deblurred=False):
    if suv_factors==None:
